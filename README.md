...
> cabclaw（微信主动推送）的官方模块仓库：业务模块独立于主项目高频发布，从源安装即用（web 或拷贝到 `modules/`），模块自带设置声明与自测。

cabclaw（微信主动推送）的**官方模块仓库**。模块更新频率高，独立于主项目（cabclaw）发布；主项目只提供平台（调度引擎/公共库/模块管理），业务能力全部由这里的模块提供。
...
cp -r <module> <cabclaw>/modules/

# 2. 启用（token 由 register 生成，模块包不含 token）
python3 <cabclaw>/modules/register.py --enable <module>
```
...
```bash
# cwd = 模块源仓库根（~/cabclaw-modules）
CABCLAW_HOST=<cabclaw 项目根> \
    python3 -m pytest todo/tests/
```

未设置 `CABCLAW_HOST` 时测试自动跳过（提示缺少主项目 common）。
